	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (!message.IsSenderSelf) {
			...
			try{
				...
				int width = (int) getContext().getResources().getDimension(org.pepal.R.dimen.chat_message_item_thumb_size);
				Picasso.with(getContext()).load(SettingSchema.mBaseRestUrl + "uploaded/profiles/" + message.SenderID + ".png").resize(width, width)
				  .centerInside().placeholder(org.pepal.R.drawable.ic_default_user_picture).error(org.pepal.R.drawable.ic_default_user_picture).into(image);
