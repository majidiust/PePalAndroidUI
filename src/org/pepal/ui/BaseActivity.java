...
import org.pepal.transport.websocket.IWebsocketServiceCallback;
import org.pepal.transport.websocket.IWebsocketService;
import org.json.JSONArray;
import org.json.JSONObject;
import org.pepal.R;
import org.pepal.schema.FriendSchema;
import org.pepal.schema.MessageSchema;
import org.pepal.schema.ProfileSchema;
import org.pepal.schema.RoomSchema;
import org.pepal.schema.SessionStore;
import org.pepal.schema.SettingSchema;
import org.pepal.transport.INGN;
import org.pepal.transport.ITransport;
		...
		try{
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try{
						//1. Get the thumbnail
						int width = (int) getResources().getDimension(R.dimen.user_profile_thumbnail_size);
						Bitmap image = Utilities.getPictureThumbnail(This, path, width);
						//2. 
						File pictureFileDirectory = new File(Environment.getExternalStoragePublicDirectory(
								Environment.DIRECTORY_PICTURES).getPath() + "/ertebat/tmp");
